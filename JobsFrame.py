...
class JobsFrame(wx.Frame):
    ...
    def Show(self):
        if self.jobs is None:
            self.jobs = {}
            jobs = db.GetJobs()
            job_codes = sorted(jobs.keys())
            for job_code in job_codes:
                job = jobs[job_code]
                if dget(job, 'active', False):
    ...
    def fill_frame(self):
        ...
        def safe_set_text(text_control, field):
            text_control.SetValue(common.dget(job, field, ""))
        ...
        #princ(job)
        safe_set_text(self.text_address, 'address')
        #self.text_address.SetValue(job['address'])
        safe_set_text(self.text_comments, 'Comments')
        ...
        safe_set_text(self.text_references, 'references')
